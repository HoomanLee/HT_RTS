...
void NuiImageBuffer::CopyDepth(const BYTE* pImage, UINT size, BOOL nearMode, DEPTH_TREATMENT treatment, DWORD width, DWORD height, RGBQUAD *pQuad, LONG *pMappedCoord, RGBQUAD* pEndAddr)
{
	m_srcWidth = width;
	m_srcHeight = height;
	...
	// sizeof(NUI_DEPTH_IMAGE_PIXEL): 4
	...
	{	
		...
		while (pPixelRun < pPixelEnd)
		{
			...
			UINT
				x = pMappedCoord[k], // tried with *(pMappedBits + (i * 2)),
				y = pMappedCoord[k+1]; // tried with *(pMappedBits + (i * 2) + 1);
			k += 2;	// pMappedCoord에 좌표가 x1, y1, x2, y2 ..... 이런 식으로 저장되어 있기 때문에 2씩 증가 시킴
			...
			if (x >= 0 && x < width && y >= 0 && y < height) {
				pQuad = pEndAddr - (x + y*width);

				// Get mapped color from depth-color table
				*rgbrun = m_depthColorTable[index][depth];
				BYTE* pRaw = (BYTE *)rgbrun;
				pQuad->rgbBlue = pRaw[COLOR_INDEX_BLUE];
				pQuad->rgbGreen = pRaw[COLOR_INDEX_GREEN];
				pQuad->rgbRed = pRaw[COLOR_INDEX_RED];
				pQuad->rgbReserved = pRaw[COLOR_INDEX_ALPHA];

				// Horizental 방향으로 flip하기 위함
				int calx = max(WIDTH-x-1, 0);	
				m_depthMap[y][calx] = depth;
			}
...
void NuiImageBuffer::InitDepthMap()
{
	// memory allocate
	m_depthMap = new USHORT*[HEIGHT];
	for(int i=0; i<HEIGHT; i++)
	{
		m_depthMap[i] = new USHORT[WIDTH];
		memset(m_depthMap[i], 0, sizeof(USHORT)*WIDTH);
	}
}



USHORT NuiImageBuffer::getDepthValue(int x, int y)
{
	USHORT depth;
	
	depth = m_depthMap[y][x];
...
//// File저장 코드 백업....별 쓸모는 없을 것 같지만 어딘가에는...
//
//static int flag = 0;
//
//if(!flag)
