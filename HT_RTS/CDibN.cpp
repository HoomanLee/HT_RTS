...
// Binary image에서 참 값인 놈들의 평균 좌표 계산하여 리턴
// rate는 현재 이미지 크기 대비 참인 픽셀의 비율. (0~1) 사이의 값만 허용
CPoint CDib::GetMeanCoordinateB(double rate)
{
	if(rate < 0 || rate > 1) {
		return false;
	}
	...
	CPoint cp;
	double ratio = ((double)cnt/(double)(width*height));
	//printf("ratio: %.2f, cnt: %d \n", ratio, cnt);
	if(cnt > 0 && ratio > rate) {
		cp.x = (UINT)(mx/cnt);
		cp.y = (UINT)(my/cnt);
	}else {
